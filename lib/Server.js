...
Server.prototype.onReady = function()
{
	*/
	...
	this.httpserver.on('error', function (error) {
		throw Error("Failed to listen on " + server_instance.options.bind_address + ":" +
			server_instance.options.bind_port + ": " + error);
	});
...
Server.prototype.handler = function(req, res)
{
	...
	try
	{
		data = fs.readFileSync(__dirname + "/.." + path, {encoding: 'utf8'});
	}
	catch (err)
	{
		if (err.code == 'ENOENT')
		{
			return server.send404(res, err);
		}
		else
		{
			return server.send500(res, err);
		}
	}
