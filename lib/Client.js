...
var exports = window; // netmask compatibility

define(['sockjs', 'jquery', 'flot', 'flot_time', 'handlebars', 'netmask'],
	function(SockJS, jquery, flot, flot_time, Handlebars, Netmask)
	{
		...
		Filter.Direction = function(home_networks)
		{
			...
			{
				this.home_networks.push(new Netmask(home_networks[i]));
			}
		};
			...
			key_field)
		{
			...
			return {
				...
				total_up: serie_out ? serie_out.total : "",
				rate_down: serie_in.average_kbps(),
				rate_up: serie_out ? serie_out.average_kbps() : ""
			};
		};
		...
		Controller.prototype.run = function()
		{
			...
			else
			{
				...
				if (home_networks)
				{
					...
					filters.push(new Filter.Coalesce(['ip_inside', 'direction']));
					this.options.labeller =
						this.options.labeller ||
						new module.Labeller('ip_inside');
					table_options = {
