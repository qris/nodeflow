...
define(['sockjs', 'jquery', 'flot', 'flot_time', 'handlebars', 'cjs!netmask'],
	function(SockJS, jquery, flot, flot_time, Handlebars, netmask)
	{
		'use strict';
		var module = {};
		...
		module.unique_fields = function(packet)
		{
			var unique = {};
		...
		Database.prototype.aggregation_key = function(packet)
		{
			...
			jquery.each(module.unique_fields(packet),
				function(field_name, field_value)
				{
		...
		var Aggregate = module.Aggregate = function(name, packet)
		{
			...
			this.unique = module.unique_fields(packet);
		};
			...
			promoted_keys)
		{
			...
			for (var i = 0; i < packets.length; i++)
			{
				...
				{
					if (this.options.labeller)
					{
						this.label_for_key[key] =
							this.options.labeller.label(packet);
					}
					else
					{
		...
		var Controller = module.Controller = function(options)
		{
			...
			this.options = jquery.extend({
				...
				home_network_template: "<li>" +
					...
					"value='{{netmask}}' /> " +
					"<a href='#' class='netgraph-home-network-remove'" +
					">Remove</a></li>\n",
		...
		Controller.prototype.run = function()
		{
			...
			else
			{
				...
				{
					filters.push(new Filter.Coalesce(['ip_src']));
					this.options.labeller =
						this.options.labeller ||
						new module.Labeller('ip_src');
				}

				this.database = new Database({
					filters: filters,
					labeller: this.options.labeller
				});
			}

			this.home_network_template_compiled =
				Handlebars.compile(this.options.home_network_template);
			for (var i = 0; i < this.options.home_networks.length; i++)
			{
				this.add_home_network(this.options.home_networks[i],
					this.options.home_network_container);
			}

			this.chart = this.options.chart || new Chart();
			this.table = this.options.table || new DataTable(
				jquery.extend(table_options,
		...
		Controller.prototype.add_home_network = function(netmask_object,
			target_query)
		{
			...
			query.on('change', 'input', function(event) {
				controller.update_home_network(event, query);
			});
			query.on('click', 'a', function(event) {
				controller.remove_home_network(event, query);
			});
			return query;
		};
