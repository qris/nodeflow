...
define(['sockjs', 'jquery', 'flot', 'flot_time', 'handlebars', 'cjs!netmask',
	...
	function(SockJS, jquery, flot, flot_time, Handlebars, netmask, es6_promise)
	{
		...
		Database.prototype.get_filters = function()
		{
			return this.options.filters.slice(0);
		};
		...
		Chart.prototype.set_window_seconds = function(window_seconds)
		{
			this.options.window_seconds = window_seconds;
		};
		...
		Chart.prototype.update_x_axis = function()
		{
			...
			var chart_xaxis = {
				...
				now: time_now,
				max: this.options.xaxis.max || time_now,
		...
		var Controller = module.Controller = function(options)
		{
			...
			this.options = jquery.extend({
				...
				aggregate_select:
					jQuery('.netgraph-aggregate-select'),
				aggregate_labels: {
					'ip_src': "Source IP",
					...
					'ip_inside': "Inside IP",
					'ip_outside': "Outside IP",
					'port_inside': "Inside Port",
					'port_outside': "Outside Port",
				},
				time_start: undefined,
				time_window: 60,
				time_window_input:
					jQuery('input[name=netgraph_time_window_input]'),
			}, options);
			this.rpc_queue = {};
		};
		...
		Controller.prototype.parse_home_networks = function(value)
		{
			...
			if (networks.length > 1 || networks[0].length > 0)
			{
				return {home_networks: networks};
			}
			else
			{
				return {home_networks: []};
			}
		};
		...
		Controller.prototype.update_location_hash = function()
		{
			...
			if (this.home_networks !== undefined)
			{
				params.push("home_networks=" +
					this.home_networks.join(","));
			}

			console.assert(this.aggregate,
				"aggregate should be set to something by " +
				"this point");
			params.push("aggregate=" + this.aggregate);

			if (this.time_window !== undefined)
			{
				params.push("time_window=" + this.time_window);
			}
		...
		Controller.prototype.set_home_networks = function(home_networks)
		{
			...
			if (home_networks !== undefined && home_networks.length > 0)
			{
				filters.push(new Filter.Direction(home_networks));
				aggregate = this.options.aggregate ||
					'ip_inside';
				this.aggregate_options = ['ip_inside',
					'ip_outside', 'port_inside',
					'port_outside'];
			...
			{
				aggregate = this.options.aggregate || 'ip_src';
				this.aggregate_options = ['ip_src', 'ip_dst',
					'port_src', 'port_dst'];
			...
			if (home_networks !== undefined)
			{
				for (i = 0; i < home_networks.length; i++)
				{
			...
			this.options.aggregate_select.empty();
			for (i = 0; i < this.aggregate_options.length; i++)
			{
				var value = this.aggregate_options[i],
					label = this.options.aggregate_labels[value];
				var item = jquery('<option>').val(value).text(label);
				item.appendTo(this.options.aggregate_select);
			}

			// Note: this will always change the filters and
			// labeller that we just assigned, which is not great
			// for efficiency, but not really worth fixing either.
			this.set_aggregate(aggregate);

			this.table.set_series_to_row_data(table_options.series_to_row_data);
			this.update_location_hash();
		};

		Controller.prototype.set_aggregate = function(new_aggregate)
		{
			this.aggregate = new_aggregate;
			this.options.aggregate_select.val(new_aggregate);
			this.update_location_hash();

			var new_aggregates = [new_aggregate];

			if (this.home_networks !== undefined &&
				this.home_networks.length > 0)
			{
				new_aggregates.push('direction');
			}

			return this.set_aggregate_internal(new_aggregates);
		};

		Controller.prototype.set_aggregate_internal = function(new_aggregates)
		{
			var filters = this.database.get_filters();
			var changed_filters = false;

			for (var i = 0; i < filters.length; i++)
			{
				if (filters[i] instanceof Filter.Coalesce)
				{
					filters[i] = new Filter.Coalesce(new_aggregates);
			...
			if (!changed_filters)
			{
				filters.push(new Filter.Coalesce(new_aggregates));
			}
			...
			var label_field = this.options.label_fields ||
				this.aggregate;
			if (this.labeller === undefined ||
				label_field != this.labeller.get_label_field())
			{
		...
		Controller.prototype.run = function()
		{
			...
			this.time_start = this.options.time_start;
			this.time_window = this.options.time_window;
			this.chart = this.options.chart || new Chart({
				window_seconds: this.time_window,
			});
			...
			if (this.options.home_networks === undefined)
			{
			...
			var controller = this;
			this.options.aggregate_select.change(function(event) {
				controller.set_aggregate(jQuery(event.target).val());
			});

			this.options.time_window_input.change(function(event) {
				var value = jQuery(event.target).val();
				controller.time_window = value;
				controller.chart.set_window_seconds(value);
				controller.update_location_hash();
		...
		module.Labeller.prototype.get_label_field = function()
		{
