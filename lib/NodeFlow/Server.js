...
context.on('ready', function() {
	...
	sjs.on('connection', function(connection) {
		...
		worker.connect('acct_1', function() {
			var ch = worker.ch;
			var ok = ch.assertExchange(exchange, 'direct', {durable: false});
			ok = ok.then(function() {
				log.info("creating queue");
				return ch.assertQueue('', {exclusive: true, autoDelete: true});
			});
			ok = ok.then(function(qok) {
				log.info("created queue " + qok.queue + " and bound " +
					"to exchange " + exchange);
				return ch.bindQueue(qok.queue, exchange, 'acct').then(function() {
					return qok.queue;
				});
			});
			ok = ok.then(function(queue) {
				log.info("consuming messages from: " + queue);
